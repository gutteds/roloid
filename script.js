document.addEventListener('DOMContentLoaded', function () {
    ...
    const gifElement = document.querySelector('#background-blur');

    if (!dynamicUI) {
        console.error('Dynamic UI element not found!');
        return;
    }

    enterText.addEventListener('click', function () {
        clickToEnter.style.opacity = '0';
        clickToEnter.style.pointerEvents = 'none';

        setTimeout(() => {
            dynamicUI.style.opacity = '1';
            dynamicUI.style.pointerEvents = 'auto';
            audio.play();
        }, 500);
    });
...
const words = ["beamed by roloid",  "brawlhalla legend", "hi i gutted", "developed by roloid - https://roloid.lol/"];
let wordIndex = 0;
let letterIndex = 0;
